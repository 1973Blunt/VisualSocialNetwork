...
class Neo4jDBPipleline(object):
    ...
    def process_item(self, item, spider):
        ...
        if isinstance(item, InformationItem):
            item["wb_usr_id"] = int(item["wb_usr_id"])
            ...
            usr = Node("WeiboUser", wb_usr_id=int(weibo["wb_usr_id"]))
            ...
            fan = Node("WeiboUser", wb_usr_id=int(followsItems["wb_usr_id"]))
            for sb_id in follows:
                sb = Node("WeiboUser", wb_usr_id=int(sb_id))
                fan_follow_sb = Relationship(fan, "FOLLOWS", sb)
                self.db.merge(fan_follow_sb)
        elif isinstance(item, FansItem):
            fansItems = dict(item)
            fans = fansItems.pop("fans")
            sb = Node("WeiboUser", wb_usr_id=int(fansItems["wb_usr_id"]))
            for fan_id in fans:
                fan = Node("WeiboUser", wb_usr_id=int(fan_id))
                fan_follow_sb = Relationship(fan, "FOLLOWS", sb)
                self.db.merge(fan_follow_sb)
        return item
