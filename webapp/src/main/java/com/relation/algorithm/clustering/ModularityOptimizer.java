 ...
 */
package com.relation.algorithm.clustering;
...
public class ModularityOptimizer {
	public static void main(String[] args) throws IOException {
		...
		resolution2 = ((modularityFunction == 1)
				? (resolution / (2 * network.getTotalEdgeWeight() + network.totalEdgeWeightSelfLinks)) : resolution);
	...
	public static Clustering cluster(int nNodes, int[][] edge) {
		if (nNodes <= 0 || edge == null || edge[0].length <= 0)
			return null;
		...
		// config
		modularityFunction = 1; // Modularity function (1 = standard; 2 =
								// alternative)
		resolution = 1.0; // Resolution parameter (e.g., 1.0)
		algorithm = 1; // Algorithm (1 = Louvain; 2 = Louvain with multilevel
						// refinement; 3 = smart local moving)
		nRandomStarts = 10; // Number of random starts (e.g., 10)
		nIterations = 10; // Number of iterations (e.g., 10)
		randomSeed = 0; // Random seed (e.g., 0)
		printOutput = true;
		...
		network = buildNetwork(nNodes, edge, modularityFunction);
